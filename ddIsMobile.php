 ...
 * 
 * Спасибо автору MobileTemplateSwitcher plugin (http://modx.com/extras/package/mobiletemplateswitcher) за идеи и чуток кода.
 * 
 * @param defaultResult {string} - This string is returning if user is not browsing with a mobile device. Default: '0'.
 * @param mobileResult {string} - This string is returning if user is browsing with a mobile device. Default: '1'.
 ...
 * 
 * @link http://code.divandesign.biz/modx/ddismobile/1.0
 * 
...
$useCookie = (isset($useCookie) && $useCookie == '0') ? false : true;
$defaultResult = isset($defaultResult) ? $defaultResult : '0';
$mobileResult = isset($mobileResult) ? $mobileResult : '1';

//Если можно использовать куки и нужная кука стоит
//TODO: При множественных вызовах в пределах одного раза, браузер будет постоянно определяться, т.к. кука установится только в следующий раз. Как вариант: хранить либо глобальную переменную, либо в ddTools.
if ($useCookie && isset($_COOKIE[$cookieName])){
	//Просто возвращаем результат в соответствии с кукой
	$mobile = ($_COOKIE[$cookieName] == 1) ? true : false;
//В противном случае, пытаемся определить
}else{
	//По умолчанию — не мобильник
	$mobile = false;
	
	$user_agent = $_SERVER['HTTP_USER_AGENT'];
	$accept = $_SERVER['HTTP_ACCEPT'];
	
	//Это странно, но будет полезно, если вдруг понадобится отдельно обработать айфоны, виндо-фоны и пр.
	switch(true){
		//iphone or ipot
		...
		//only mobile devices giving a HTTP_X_WAP_PROFILE or HTTP_PROFILE header
	...
	//Если можно, поставим куку на 5 лет
	if ($useCookie) setcookie($cookieName, ($mobile ? 1 : 0), time() + 60 * 60 * 24 * 365 * 5, '/');
}
